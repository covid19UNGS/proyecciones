...
* Los datos cargados a partir de los reportes del Ministerio de Salud, tienen diferencias con los "Datos abiertos del Ministerio de Salud" en este último caso utilizamos las fechas de diagnóstico y la provincia de residencia, cuando la provincia de residencia está SIN ESPECIFICAR se toma la provincia de carga. 
...
corI1 %>% group_by(residencia_provincia_nombre) %>% summarise(fecha=max(fecha), Abiertos= sum(Abiertos, na.rm = TRUE), Reportes = sum(Reportes, na.rm = TRUE)) 
