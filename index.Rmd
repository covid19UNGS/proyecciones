...
cor_FallRec <- cor_conf %>% filter(fallecido=="SI") %>% group_by(carga_provincia_nombre,hospitalizado) %>%
  summarise(edad=median(edad, na.rm = TRUE),dias_hospitalizacion=median(dias_hospitalizacion, na.rm = TRUE),n=n()) %>%
  mutate(Porcentaje = round(n / sum(n) * 100)) %>% filter(!hospitalizado)
...
cor_FallRec <- cor_conf  %>% filter(hospitalizado) %>%
  group_by(carga_provincia_nombre,fallecido) %>% 
  ...
  mutate(Porcentaje = round(n / sum(n) * 100)) %>% filter(fallecido=="SI") 

knitr::kable(select(cor_FallRec, -fallecido),caption = "Tabla 2: Proporcion Fallecidos con respecto a hospitalizados")


cor_FallRec <- cor_conf  %>%  group_by(carga_provincia_nombre,hospitalizado) %>% 
...
knitr::kable(select(cor_FallRec, - hospitalizado),caption = "Tabla 2: Proporcion Hospitalizados")

# cor_FallRec <- cor_conf %>% filter(carga_provincia_nombre == "Mendoza")

corLetalidad <- cor_conf %>% group_by(carga_provincia_nombre,fallecido) %>% summarise(n=n()) %>%mutate(Porcentaje = round( n / sum(n) * 100)) %>% filter(fallecido=="SI")
