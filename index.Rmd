...
unique(cor_nac$carga_provincia_nombre)
corR <- cor_nac %>% filter(clasificacion_resumen=="Confirmado") %>% 
      mutate(fecha=fecha_diagnostico) %>% filter(carga_provincia_nombre %in% c("Chaco","CABA","Buenos Aires","Neuquén", "Río Negro")) %>%
      group_by(carga_provincia_nombre ) %>% 
      do( {
...
ggplot(corR, aes(fecha,medianR)) + geom_line() + geom_ribbon(aes(ymin=quantile025,ymax=quantile975),alpha=0.2) +
  ...
  facet_wrap(~carga_provincia_nombre) + geom_vline(data= fases, aes(xintercept = fecha), col = "red", lty = 3)  +
  geom_text(data = fases, mapping = aes(label = nombre, x=fecha,y = 0), angle = 60, hjust = 0,size=3) + ylab("R effectivo (7 Días)") + xlab("")
