...
#cor_nac <- read_csv(csv_fname1,locale = locale(encoding = "ISO-8859-1"))
...
* Si tomamos los "Datos abiertos del ministerio de salud", la diferencia está en la posibilidad de determinar la fecha del inicio de los síntomas <http://datos.salud.gob.ar/dataset/covid-19-casos-registrados-en-la-republica-argentina/archivo/fd657d02-a33a-498b-a91b-2ef1a68b8d16>
...
cor_incidence_obj <- incidence::incidence(cor_incidence_obj$fecha,
                                          last_date=ymd(max(corConf$ultima_actualizacion)))
...
dfCABA <- cor %>% filter( osm_admin_level_4 =="Tierra del Fuego", is.na(osm_admin_level_8) | osm_admin_level_8!="Malvinas") 
esProv <- estima_Re_from_df(dfCABA,"Tierra del Fuego",end_date = max(cor$fecha))
```
...
# 
# esProv <- estima_Re_from_df(cor_incidence_obj,"TDF Locales/Importados Fecha Síntomas")
