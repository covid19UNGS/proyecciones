...
cor_nac <- read_csv(x_utf8) %>% mutate(residencia_provincia_nombre=ifelse(residencia_provincia_nombre=="SIN ESPECIFICAR",carga_provincia_nombre,residencia_provincia_nombre))
...
dfArg <- cor %>% filter( osm_admin_level_4 =="Indeterminado") 
cor_incidence <- dfArg  %>% mutate(casos_dia=tot_casosconf - lag(tot_casosconf)) %>% dplyr::select(fecha, casos_dia)
cor_incidence$casos_dia[1] <- 1
cor_incidence <- cor_incidence %>% uncount(casos_dia)
...
ar_res_parametric_si <- estimate_R(cor_incidence_obj, 
                                   method = "uncertain_si", 
...
empirical_si_config <- make_config(list(si_parametric_distr = dist, 
    ...
    n2 = 50))
ar_res_empirical_si <- estimate_R(cor_incidence_obj, method = "si_from_data", 
...
esProv <- estima_Re_from_df(dfCABA,"CABA",end_date = max(cor$fecha),fases=fases)
...
esProv <- estima_Re_from_df(cor_incidence_obj,"CABA Datos Abiertos MinSal",fases=fases)
...
esProv <- estima_Re_from_df(dfCABA,"Buenos Aires",end_date = max(cor$fecha),fases=fases)
...
corR <- cor_nac %>% filter(clasificacion_resumen=="Confirmado") %>% 
      mutate(fecha=fecha_diagnostico,residencia_provincia_nombre=ifelse(residencia_provincia_nombre=="SIN ESPECIFICAR",carga_provincia_nombre,residencia_provincia_nombre)) %>% filter(residencia_provincia_nombre %in% c("Chaco","CABA","Buenos Aires","Neuquén", "Río Negro","Chubut","Jujuy")) %>%
      ...
      do( {
          ...
          esProv <- estima_Re_from_df(cor_incidence_obj,"Datos Abiertos MinSal",fases=fases,plot = FALSE)
...
esProv <- estima_Re_from_df(cor_incidence_obj,"Datos Abiertos MinSal",fases=fases,plot = TRUE)
...
```{r Incid-provincia-TDF_RioGrande, echo=FALSE, tidy=TRUE, message=FALSE, warning=FALSE,cache.extra = tools::md5sum(csv_fname1)}

corI <- cor_nac %>% mutate(fecha=ymd(fecha_apertura),
                           residencia_provincia_nombre=ifelse(residencia_provincia_nombre=="SIN ESPECIFICAR",carga_provincia_nombre,residencia_provincia_nombre)) %>% filter(clasificacion_resumen=="Confirmado", residencia_provincia_nombre %in% c("Tierra del Fuego"),fecha < ultima_actualizacion,residencia_departamento_nombre=="Río Grande") %>% group_by(fallecido) %>% summarise(fecha=max(fecha),n=n(), porMillon=n/88700*1000000)

knitr::kable(corI,digits = 0)

corI <- cor_nac %>% mutate(fecha=ymd(fecha_apertura),
                           residencia_provincia_nombre=ifelse(residencia_provincia_nombre=="SIN ESPECIFICAR",carga_provincia_nombre,residencia_provincia_nombre)) %>% filter(clasificacion_resumen=="Confirmado", residencia_provincia_nombre %in% c("Tierra del Fuego"),fecha < ultima_actualizacion,residencia_departamento_nombre=="Río Grande")
cor_incidence_obj <- select(corI , fecha)
cor_incidence_obj <- incidence::incidence(cor_incidence_obj$fecha,
                                          last_date=ymd(max(corI$ultima_actualizacion))-1)
...
corI <- cor_nac %>% mutate(fecha=ymd(fecha_apertura),
                           residencia_provincia_nombre=ifelse(residencia_provincia_nombre=="SIN ESPECIFICAR",carga_provincia_nombre,residencia_provincia_nombre)) %>% filter(clasificacion_resumen=="Confirmado", residencia_provincia_nombre %in% c("Chaco","CABA","Buenos Aires","Neuquén", "Río Negro","Chubut","Jujuy"),fecha < ultima_actualizacion) %>% group_by(residencia_provincia_nombre,fecha) %>% summarise( Casos = n()) %>% mutate(datos="Abiertos")
...
corI1 <- cor %>% mutate(fecha=ymd(fecha)) %>% filter(osm_admin_level_4 %in% c("Chaco","CABA","Buenos Aires","Neuquén", "Río Negro","Chubut","Jujuy"),fecha <= max(corI$fecha)) %>% select(osm_admin_level_4,fecha,nue_casosconf_diff) %>% rename(Casos=nue_casosconf_diff,residencia_provincia_nombre=osm_admin_level_4) %>% mutate(datos="Reportes")
...
#
#
cor_conf <- cor_nac %>% filter(clasificacion_resumen=="Confirmado") %>% mutate_at(vars(starts_with("fecha")),ymd) %>% 
  mutate(fecha_internacion = if_else(is.na(fecha_internacion),fecha_cui_intensivo,fecha_internacion),
...
cor_FallRec <- cor_conf %>%  group_by(residencia_provincia_nombre,hospitalizado,fallecido) %>% filter(!hospitalizado,fallecido=="SI")
  
cor_FallRec <- cor_conf %>% mutate(dias_fallecimiento= fecha_fallecimiento - fecha_inicio_sintomas) %>%  group_by(residencia_provincia_nombre,hospitalizado,fallecido) %>%
  summarise(edad=median(edad, na.rm = TRUE),dias_fallecimiento=median(dias_fallecimiento, na.rm = TRUE),n=n()) %>%
  mutate(Porcentaje = round(n / sum(n) * 100,2)) %>% filter(!hospitalizado,fallecido=="SI")  %>% arrange(desc(n))

knitr::kable(select(cor_FallRec, - hospitalizado,-fallecido),caption = "Tabla 1: Fallecidos sin hospitalización con respecto a Casos, nro de días con respecto a inicio de síntomas")


cor_FallRec <- cor_conf %>%  mutate(dias_fallecimiento= fecha_fallecimiento - fecha_inicio_sintomas) %>%
  group_by(hospitalizado,fallecido) %>%
  summarise(edad=median(edad, na.rm = TRUE),dias_fallecimiento=median(dias_fallecimiento, na.rm = TRUE),n=n()) %>%
  mutate(Porcentaje = round(n / sum(n) * 100,2)) %>% filter(!hospitalizado, fallecido=="SI")  %>% arrange(desc(n)) %>% ungroup() 

knitr::kable(select(cor_FallRec, - hospitalizado),caption = "Tabla 2:  TOTAL: Fallecidos sin fecha de hospitalización con respecto a Casos, nro de días con respecto a inicio de síntomas")

#
# Fallecidos hospitalizados por categoria de edad
#
cor_FallRec <- cor_conf %>%  mutate(dias_fallecimiento= fecha_fallecimiento - fecha_inicio_sintomas, rango_edad=as.character(cut(edad,breaks=c(-Inf,15,65,Inf),labels=c("Niño 15<","Adulto 65<","Mayor")))) %>% filter(!is.na(rango_edad))  %>% group_by(rango_edad,hospitalizado,fallecido ) %>% summarise(edad=median(edad, na.rm = TRUE),dias_fallecimiento=median(dias_fallecimiento, na.rm = TRUE),n=n()) %>%
  mutate(Porcentaje = round(n / sum(n) * 100,2)) %>% filter(fallecido=="SI")   
ggplot(cor_FallRec, aes(x=fct_relevel(rango_edad, "Mayor"), y=Porcentaje, fill=hospitalizado)) + geom_bar(stat="identity") + facet_wrap(~hospitalizado) + scale_fill_brewer( palette = "Paired")
knitr::kable(select(cor_FallRec, - fallecido),caption = "Tabla 2A:  Fallecidos hospitalizados o no por categoria de edad") 
#    geom_histogram(binwidth=10, colour="black", fill="white") + ylab("Nro de Fallecimientos")


#
# Fallecidos totales por categoria de edad
#
cor_FallRec <- cor_conf %>%  mutate(dias_fallecimiento= fecha_fallecimiento - fecha_inicio_sintomas, rango_edad=as.character(cut(edad,breaks=c(-Inf,15,65,Inf),labels=c("Niño 15<","Adulto 65<","Mayor >65")))) %>% filter(!is.na(rango_edad))  %>% group_by(rango_edad,fallecido ) %>% summarise(edad=median(edad, na.rm = TRUE),dias_fallecimiento=median(dias_fallecimiento, na.rm = TRUE),n=n()) %>%
  mutate(Porcentaje = round(n / sum(n) * 100,3)) %>% filter(fallecido=="SI")   
ggplot(cor_FallRec, aes(x=fct_relevel(rango_edad, "Mayor >65"), y=Porcentaje, fill=rango_edad)) + geom_bar(stat="identity") +   scale_fill_brewer( palette = "Paired") + guides(fill=FALSE) + xlab("")
knitr::kable(select(cor_FallRec, - fallecido),caption = "Tabla 2B:  Fallecidos totales por categoria de edad") 
...
cor_FallRec <- cor_conf  %>% filter(hospitalizado) %>%
  ...
  mutate(Porcentaje = round(n / sum(n) * 100,2)) %>% filter(fallecido=="SI")  %>% arrange(desc(n))
...
knitr::kable(cor_FallRec,caption = "Tabla 4: TOTAL: Proporcion Fallecidos con respecto a hospitalizados")
...
knitr::kable(select(cor_FallRec, - hospitalizado),caption = "Tabla 6: TOTAL Proporción Hospitalizados con respecto a casos")


# cor_FallRec <- cor_conf %>% filter(residencia_provincia_nombre == "Mendoza")

corLetalidad <- cor_conf %>% group_by(residencia_provincia_nombre,fallecido) %>% summarise(n=n()) %>%mutate(Porcentaje = round( n / sum(n) * 100,2)) %>% filter(fallecido=="SI")  %>% arrange(desc(n))

knitr::kable(select(corLetalidad, -fallecido),caption = "Tabla 7: Letalidad Proporción Fallecidos con respecto a casos")

corLetalidad <- cor_conf %>% group_by(fallecido) %>% summarise(n=n()) %>%mutate(Porcentaje = round( n / sum(n) * 100,2)) %>% filter(fallecido=="SI")
knitr::kable(corLetalidad,caption = "Tabla 8: TOTAL Letalidad Proporción Fallecidos con respecto a casos")
...
cor_FallRec <- cor_conf  %>% filter(hospitalizado) %>%
  ...
  summarise(edad=median(edad, na.rm = TRUE),dias_hospitalizacion=median(dias_hospitalizacion, na.rm = TRUE),
            dias_pre_hospitalizacion=median(dias_pre_hospitalizacion, na.rm = TRUE), n=n()) %>%
  mutate(Porcentaje = round(n / sum(n) * 100,2)) 
knitr::kable(cor_FallRec,caption = "Tabla 9: TOTAL: Proporcion Cuidado Intensivo con respecto a hospitalizados")
...
cor_FallRec <- cor_conf  %>% filter(hospitalizado) %>%
  ...
  summarise(edad=median(edad, na.rm = TRUE),dias_hospitalizacion=median(dias_hospitalizacion, na.rm = TRUE),
            dias_pre_hospitalizacion=median(dias_pre_hospitalizacion, na.rm = TRUE), n=n()) %>%
  mutate(Porcentaje = round(n / sum(n) * 100,2))  
knitr::kable(cor_FallRec,caption = "Tabla 10: TOTAL:  De Hospitalizados cuantos Fallecen y fueron a Cuidado Intensivo, o no")
...
cor_FallRec <- cor_conf  %>% filter(cuidado_intensivo=="SI") %>%
  ...
  summarise(edad=median(edad, na.rm = TRUE),dias_hospitalizacion=median(dias_hospitalizacion, na.rm = TRUE),
            dias_pre_hospitalizacion=median(dias_pre_hospitalizacion, na.rm = TRUE), n=n()) %>%
  mutate(Porcentaje = round(n / sum(n) * 100,2))  
knitr::kable(cor_FallRec,caption = "Tabla 11: TOTAL:  De Cuidado intensivo cuantos Fallecen")
